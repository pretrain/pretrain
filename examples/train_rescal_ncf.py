...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=str,
        default="../configs/rescal_ncf_default.json",
...
class NCF_train(TrainEngine):
    ...
    def train_rescal(self):
        ...
        self.engine.model.embedding_user_mf.weight.data = torch.tensor(u_emb.astype(np.float64)).to(self.engine.device)
        self.engine.model.embedding_item_mf.weight.data = torch.tensor(i_emb.astype(np.float64)).to(self.engine.device)
