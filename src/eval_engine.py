...
from src.utils.common_util import print_dict_as_table, save_to_csv, timeit
...
def test_eval_worker(testEngine, eval_data_df, prediction, k_li=[5, 10, 20]):
    ...
    if "n_bases" in testEngine.config:
        result_para["n_bases"] = [int(testEngine.config["n_bases"])]
    if "opn" in testEngine.config:
        result_para["opn"] = [testEngine.config["opn"]]
    ...
    if "reg" in testEngine.config:
        result_para["reg"] = [testEngine.config["reg"]]
    if "loss" in testEngine.config:
        result_para["loss"] = [testEngine.config["loss"]]
    if "dropout" in testEngine.config:
        result_para["dropout"] = [testEngine.config["dropout"]]
...
class EvalEngine(object):
    ...
    def train_eval(self, valid_data_df, test_data_df, model, epoch_id=0, k=10):
        ...
        """
        valid_pred = (
            self.predict(valid_data_df, model) if valid_data_df is not None else None
        )
