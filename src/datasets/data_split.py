...
from src.utils.common_util import get_dataframe_from_npz, save_dataframe_as_npz
...
def load_split_data(path, n_test=10, validate=1):
...
def leave_one_out(data, random=False, validate=1):
    """leave_one_out split
...
def split_data(
    ...
    if split_type == "random":
        ...
        data = leave_one_out(data, random, validate)
    ...
    if validate:
        save_split_data(
            tp_validate, save_dir, split_type, parameterized_path, "valid.npz"
        )
    ...
    if n_items - valid_neg_max < n_negative or n_items - test_neg_max < n_negative:
        raise RuntimeError(
            ...
            + (f"valid_neg_max: {n_items-valid_neg_max}, " if validate else "")
...
def generate_parameterized_path(
    test_rate=0, random=False, n_negative=100, by_user=False, validate=1
):
    ...
    path_str += "_validate" if validate else ""
    return path_str
