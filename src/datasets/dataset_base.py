...
from src.utils.constants import DEFAULT_ORDER_COL, DEFAULT_TIMESTAMP_COL
from src.utils.download import download_file, get_format
...
class DatasetBase(object):
    ...
    def make_leave_one_out(
        self, data=None, random=False, n_negative=100, n_test=10, validate=1
    ):
    ...
    def load_leave_one_out(self, random=False, n_negative=100, n_test=10, validate=1):
        ...
        if not os.path.exists(processed_leave_one_out_path):
            self.make_leave_one_out(
                random=random, n_negative=n_negative, n_test=n_test, validate=validate
            )

        # load data from local storage
        return load_split_data(
            processed_leave_one_out_path, n_test=n_test, validate=validate
        )
    ...
    def load_split(self, config):
        ...
        split_paras["validate"] = config["validate"] if "validate" in config else 1
        ...
        split_para_mapping = {
            "leave_one_out": ["random", "n_negative", "n_test", "validate"],
            "leave_one_basket": ["random", "n_negative", "n_test", "validate"],
            "random_split": [
                "test_rate",
                "by_user",
                "n_negative",
            ...
            "temporal": ["test_rate", "by_user", "n_negative", "n_test", "validate"],
            "temporal_basket": [
                "test_rate",
                "by_user",
                "n_negative",
